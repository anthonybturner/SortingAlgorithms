...
public class Quicksort
{
   ...
   * */
   public static int quicksort(int[ ] data, int first, int n)
   {
      ...
      if (n > 1)
      {
         // Partition the array, and set the pivot index.
    	  pivotIndex = partition(data, first, n, comparisons);
        ...
        comparisons+= quicksort(data, first, n1);
        comparisons+= quicksort(data, pivotIndex + 1, n2);
      }
      comparisons++;//The if statement comparison
   ...
   private static int partition(int[ ] data, int first, int n, Integer comparisons){
	   ...
	   while( tooBigIndex < tooSmallIndex ){
		   ...
		   while( data[tooBigIndex] <= pivot_value ){
			   tooBigIndex++;
		   }
		   
		   while( data[tooSmallIndex] > pivot_value){
		   ...
		   showData(data, pivot_value, tooBigIndex, tooSmallIndex);
	   }
	   
	   //Make the  value that has moved to the left of the pivot
	   //The new pivot vale
	   //The old pivot is shifted towards the middle of the partition
	   //Where the too small value was
	   data[first] = data[tooSmallIndex];
	   data[tooSmallIndex]= pivot_value;

      return tooSmallIndex; 
   }


   private static void showData(int[] data, int pivot, int bigIndex, int smallIndex) {
